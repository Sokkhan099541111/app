...
useEffect(() => {
  ...
  const loadUnitGroups = async () => {
        try {
            const response = await fetch('http://127.0.0.1:8080/api/objects/group_snkrp"');
            if (!response.ok) {
...
const fetchData = async () => {
  ...
  try {
    ...
    const response = await fetch(`http://127.0.0.1:8080/api/reports/vehicles?${params}`);
...
{/* <select id="unitGroup" value={selectedUnitGroup} onChange={e => setSelectedUnitGroup(e.target.value)} style={{ padding: '5px', fontSize: '12px', minWidth: '100px' }}>
